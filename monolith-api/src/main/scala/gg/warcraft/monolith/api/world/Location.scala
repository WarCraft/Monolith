...
case class Location(
  ...
  def distanceTo(target: Location): Float = translation.distanceTo(target.translation)

  def toBlockLocation: BlockLocation = BlockLocation(world,
    Vector3i(Math.floor(x).toInt, Math.floor(y).toInt, Math.floor(z).toInt))
