...
class PlayerDataHandler(implicit service: PlayerDataService) extends Event.Handler {
  ...
  private def handleDisconnect(event: PlayerDisconnectEvent): Unit = {
    service.updatePlayerData(event.player.id)
    service.invalidatePlayerData(event.player.id)
  }
}
