...
class BlockLocationSpec extends FlatSpec with GivenWhenThen {

  "A SimpleBlockLocation" should "expose its world" in {
    val location = random.world.blockLocation()
  ...
  it should "add a location to itself" in {
    ...
    Given("a location")
    val other = random.world.blockLocation()
  ...
  it should "map itself to a Location" in {
    val location = random.world.blockLocation()
